...
\hypersetup{
    ...
    citecolor=brandblue,
    urlcolor=brandblue,
    pdftitle={A DSL for Geometric Simplicial Complexes},
    pdfauthor={Simone Riccio}
}
...
% Stile personalizzato per i teoremi/definizioni
\newtcbtheorem[number within=section]{mydef}{Definizione}{
    colback=brandblue!5, 
    colframe=brandblue, 
    fonttitle=\bfseries,
    boxrule=0.5pt,
    leftrule=3pt, % Linea spessa solo a sinistra
    arc=2pt,
...
\lstdefinestyle{mystyle}{
    ...
    basicstyle=\ttfamily\small,          % Font monospace leggibile
    ...
    frame=single,                        % Cornice sottile intorno al codice
...
\begin{definition} [Simplex]
    Given a set of $k+1$ affinely independent points in $\R^n$ denoted as $\left\{v_0, v_1, \ldots, v_k \right\}$, the $k$-simplex $\sigma$ spanned by these points is defined as the convex hull:
    \[
    \sigma = \left[v_0, v_1, \ldots, v_k\right] = \left\{ \sum \lambda_i v_i \mid \sum \lambda_i = 1, \lambda_i \ge 0 \right\}
    \]
\end{definition}
...
Modern developments in spatial logic extend these ideas to discrete spatial structures (like graphs or images) using a generalized topological space called a closure space~\cite{closure-spaces-lmcs}.
...
\begin{definition} [Spatial Logic for Closure Spaces \cite{closure-spaces-lmcs}]
    In a closure space (X, C), the syntax of the spatial logic is defined as follows:
    \[
...
This specific formulation guarantees that operations like union and intersection can directly leverage highly optimized set-theoretic operations at the Python interpreter level (\ref{lst:core_structs}).
...
To achieve this, we used a \texttt{\_build\_downward\_closure} algorithm. For every complex marked by a \texttt{render\_targets} directive, the algorithm extracts all maximal simplices and systematically generates all possible sub-simplices using Python's \texttt{itertools.combinations} (\ref{lst:downward_closure}). 

\begin{lstlisting}[language=Python, caption={Downward Closure Generation}, label = {lst:downward_closure}]
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{webinterface/3d.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{webinterface/3dquery.png}
...
The code is similar and intuitive, for example in Figure \ref{fig:web-interface}
...
function clone_and_shift(target, factor, dx, dy, dz) {
    complex temp = scale(target, factor)
    complex shifted_result = translate(temp, (dx, dy, dz))
...
\section{Examples and Model Checking}
    ...
    We define a simple 3D simplicial complex representing two tetrahedra sharing an edge. The DSL code snippet is as follows:
...
This will generate the simplicial complex seen in Figure \ref{fig:example1-plain}. The downward closure of this complex will include all vertices, edges, and faces, which will be serialized into a poset for PolyLogicA. We can then apply spatial logic queries to this complex using PolyLogicA. For example:
...
And the result of this query is shown in Figure \ref{fig:example1-query}. The intersection of the two simplices is a single edge, which is highlighted in green in the figure.

\begin{figure}[h]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{ex4/plain.png}
        \caption{Result of the render of Example 1}
        \label{fig:example1-plain}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{ex4/q.png}
...
point p2 = (1, 1.7, 0)
point p3 = (1, 0.5, 1.6)
complex t1 = scale([p0, p1, p2, p3], 4)
...
The resulting complex is shown in Figure \ref{fig:example2}. It consists of four tetrahedra, with some of them sharing faces and vertices.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{example4/plain.png}
    \caption{Resulting Complex from Example 2}
    \label{fig:example2}
\end{figure}

And we can apply spatial logic queries to this complex to check for connected components by using the $\mathcal{U}$ operator, which computes the closure of a complex with respect to another complex. For example, we can check if $together$ is connected to $t2$ by computing the closure of $together$ with respect to $t2$:
...
\begin{figure}[h]
    ...
    \begin{minipage} {0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{example4/test1.png}
        \caption{Result of the Query for Example 2 with scale factor = 4}
        \label{fig:example2-query}
    \end{minipage}
    \begin{minipage} {0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{example4/test2.png}
        \caption{Result of the Query for Example 2 with scale factor = 5}
        \label{fig:example2-query2}
    \end{minipage}
\end{figure}
...
Furthermore, another promising avenue for future research is to interface our simplicial representations with geometric model checking techniques designed specifically for continuous spaces~\cite{geometric-model-checking}. This integration would pave the way for bridging the gap between discrete, combinatorial simplicial complexes and the analytical properties of continuous geometric structures.
...
\begin{thebibliography}{9}
    ...
    \bibitem{closure-spaces-lmcs}
    Vincenzo Ciancia, Diego Latella, Michele Loreti, and Mieke Massink,
    \textit{Model Checking Spatial Logics for Closure Spaces},
    \textit{Logical Methods in Computer Science}, 12(4), 2016.

    \bibitem{voxlogica}
    Gina Belmonte, Vincenzo Ciancia, Diego Latella, and Mieke Massink,
    \textit{VoxLogicA: A Spatial Model Checker for Declarative Image Analysis},
    In \textit{International Conference on Tools and Algorithms for the Construction and Analysis of Systems} (TACAS), pp. 281--298, 2019.

    \bibitem{geometric-model-checking}
    Nick Bezhanishvili, Vincenzo Ciancia, David Gabelaia, Gianluca Grilletti, Diego Latella, and Mieke Massink,
    \textit{Geometric Model Checking of Continuous Space},
    \textit{Logical Methods in Computer Science}, 18(4), 2022.

    \bibitem{belmonte2025symbolic}
    Gina Belmonte, Vincenzo Ciancia, and Mieke Massink,
    \textit{Symbolic and hybrid AI for brain tissue segmentation using spatial model checking},
    \textit{Artificial Intelligence in Medicine}, 167: 103154, 2025.
\end{thebibliography}
